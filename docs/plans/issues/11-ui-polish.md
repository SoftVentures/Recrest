...
> ✅ **Erledigt** (mit Abweichungen vom ursprünglichen Plantext):
> - **Task 1 Logo-Nav:** Logo in `BrandLink` (NavLink → Dashboard) mit Hover/Focus, aria `sidebar.home`. ✓
> - **Task 2 Achse:** Betrifft die **HeatmapCard** (Stunden×Wochentage), nicht ActivityChart. Stundenachse zeigt jetzt `0,6,12,18,24` (24 als Overlay-Cap am rechten Plot-Rand). Sonne/Mond-Icons bewusst **weggelassen** (User-Entscheidung — nur Achse fixen).
> - **Task 3 Ligaturen:** UI auf `GeneralSwitchInput` (on/off) reduziert; mappt auf das bestehende String-Format (`an→"standard"`, `aus→"off"`) — **keine** Rust-/Typ-/Migrations-Änderung, rückwärtskompatibel.
> - **Task 4 Shortcuts:** Komplett **neu vergeben** (User-Wunsch „Shortcuts, die mehr Sinn machen"): ⌘1–6 Navigation, ⌘, Settings, ⌘K Suche, ⌘⇧F Repo-übergreifend, ⌘B Sidebar. Eine deklarative Registry (`shortcuts.constants.ts`) speist Bindung (`useGlobalShortcuts`) **und** Anzeige (ShortcutsTab) — können nie wieder driften. ⌘R (Refresh) **bewusst entfernt** (kollidiert mit Webview-Reload; Header-Button deckt Refresh ab). Per Playwright verifiziert.
> - **Task 5 macOS-Icon:** Dunkles Icon **gefixt** — Near-Black-Tile (#0F1115) verschmolz auf dunklem Dock; Tile auf Slate **#23262E** geändert (prod + dev, mac + full-bleed), Icons regeneriert. Vorher/Nachher per Playwright auf Dock-Hintergründen verglichen. Finaler Dock-Render nur per echtem macOS-Build prüfbar.
> - **Task 6 Time-Selector:** Range-Selector im collapsed-Zustand in `GeneralTooltip` mit aktuellem Range-Label.
>
> test:ts/lint/739 Tests grün, Dark-Mode-Playwright durchgängig verifiziert.
...
- [x] **Step 1: Logo zu `<button>` umbauen**
...
- [x] **Step 2: aria-Locale-String ergänzen**
...
- [x] **Step 2: Wochentag-Icons an Y-Achsen-Enden**
...
- [x] **Step 1: Jede Zeile gegen tatsächliche Bindung prüfen**
...
- [x] **Step 1: Visual-Inspektion der vier mac-SVGs**
...
- [x] **Step 1: Tooltip im collapsed-State**
...
- [x] `yarn test:ts && yarn workspace @recrest/app test && yarn lint`
- [x] **Smoke (`yarn dev`)**:
...
- [x] **macOS-Icon-Check**: Dock + Finder zeigen beide Dreiecke in Light + Dark
