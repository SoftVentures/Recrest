...
- [ ] **1.C6** — Auf 4K-Display (Arch + Hyprland): UI in nativer Größe, keine doppelte Skalierung. Mehrere Monitore mit unterschiedlicher Skala: korrekte Größe pro Monitor. (→ Plan 1 §C.6) — **OFFEN.** Die frühere Bewertung („Wayland fractional scaling liefert korrekte native Skalierung automatisch, kein Eingriff nötig") ist durch zwei Nutzerberichte von Linux/Wayland widerlegt und faktisch falsch: der Stack ist GTK **3** (`gtk`/`gdk` 0.18.2 laut `Cargo.lock`), und `wp_fractional_scale_v1` ist nur in GTK4 implementiert — tao hält den Scale-Faktor konsequent als `AtomicI32`, hat also gar keine Repräsentation für fraktionales Scaling. Dazu kommt eine **dritte**, im Plan nicht erfasste Skalierungsebene: WebKitGTK hat das ehemalige _Text_-Scaling aus `gtk-xft-dpi` in einen **Page**-Zoom umgebaut, wodurch z. B. GNOMEs „Große Schrift" die komplette Seite zoomt und den CSS-Viewport schrumpft. Und die Startgröße ist eine harte Konstante ohne jeden Monitorbezug (kein einziger Treffer für `scale_factor`/`current_monitor`/`available_monitors` im Rust-Code). Befunde, Belege und Fix-Richtungen: `docs/plans/10-linux-scaling-and-packaging-audit.md`, Punkte 6, 7 und 8.

  **Stand 2026-08-27 — teilweise bestätigt.** Nutzerbericht nach längerem produktivem Einsatz auf Arch/Wayland (native Builds, sowohl `recrest` als auch `recrest-git`): Skalierung funktioniert, Text lesbar, keine doppelte Skalierung, keine sonstigen UI-Probleme. Das deckt die rem-Migration aus PR #118 im Alltagsbetrieb ab. **Noch nicht bestätigt** und daher weiterhin offen: der explizite 4K-Fall und mehrere Monitore mit _unterschiedlicher_ Skala — beides nennt der Bericht nicht. X11 bleibt komplett ungetestet (der Tester hat keinen X-Server), was auch 1.C5 betrifft.
...
- [x] **1.D6** — Cmd/Ctrl + `+` / `-` / `0` skaliert UI live. Slider in System-Settings reflektiert dieselbe Skala (bidirektional gesynct). Skala persistiert. (→ Plan 1 §D.6) — **erledigt in PR #118** (`zoom` → `rem`): Hotkeys in `shortcuts.constants.ts` (`ZOOM_IN`/`ZOOM_OUT`/`ZOOM_RESET`, inkl. `+`/`=`/`_`-Layoutvarianten), Regler „Oberflächengröße" in den Einstellungen (0,8–1,5 in 0,05-Schritten), beide auf demselben `settings.uiScale`; persistiert über `settings.json` und für den ersten Frame in `localStorage` gespiegelt. Der historische Befund von damals: Verifiziert am Code: (1) **keine Zoom-Hotkeys** — `app/src/lib/constants/shortcuts.constants.ts` enthält keine Zoom-/Scale-Aktion; (2) **kein Slider** — nur ein 4-stufiges Schriftgrößen-Select (`FONT_SIZES = ["sm", "md", "lg", "xl"]`, `shared/src/constants/ui.ts:78`), das über `scaleForSize()` (`app/src/theme/ThemeWrapper.tsx:31-42`) auf 0.94/1/1.12/1.25 abbildet und als globaler CSS-`zoom` auf `#root` landet (`app/src/styles/globals.css:31-38`) — also UI-Zoom, nicht Schriftgröße; (3) **`uiScale` ist toter Code im Renderer** — in `shared/src/types/settings.ts:160` deklariert und via Rust persistiert, aber außer Fixtures/Seeds ohne Konsumenten. Folgefehler (Media Queries sehen den Zoom nicht, portalierte MUI-Overlays skalieren nicht mit): `docs/plans/10-linux-scaling-and-packaging-audit.md`, Punkte 4, 5, 9 und 12.
