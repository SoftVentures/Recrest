...
- [x] **Step 1: Add a committed fixture**
...
- [x] **Step 3: Run to confirm failure** — author is currently `"ada"` (username), not `"Ada Lovelace"`.

- [x] **Step 4: Fix the mapping**
...
- [x] **Step 5: Run the test** → PASS. **Step 6: Commit** (`feat: GitLab MR author shows real name + avatar (D.1)`).
...
- [x] **Step 1:** Commit a fixture with `author.display_name` + `author.links.avatar.href`.
- [x] **Step 2:** Write a wiremock test asserting `prs[0].author == "<display_name>"` and `author_avatar_url.is_some()`.
- [x] **Step 3:** Run → PASS (no code change expected; if it fails, fix the mapping). **Commit** (`test: lock in Bitbucket author/avatar mapping (D.1)`).
...
- [x] **GitLab:** `GlMr` already returns `assignees[]` / `reviewers[]` with `name`/`username`. Map their names into `PullRequestDto.assignees` / `requested_reviewers`. Add a test asserting non-empty when the fixture has them.
- [x] **Bitbucket:** `reviewers[]` present on the PR list payload (`reviewers[].display_name`); map into `requested_reviewers`. (Bitbucket has no MR "assignees" → leave empty.) Test.
- [x] Commit (`feat: map GitLab/Bitbucket assignees + reviewers (D.1)`).
...
- [x] **Step 1:** Commit fixtures: `tests/fixtures/gitlab/groups.json` (`GET /groups?membership=true`) and `tests/fixtures/bitbucket/workspaces.json` (`GET /workspaces?role=member`).
- [x] **Step 2:** Write wiremock tests:
...
- [x] **Step 3:** Run → PASS (fix mapping if a test fails). Also add a `list_repositories_for_org` test for each:
  - GitLab: `GET /groups/:id/projects`.
  - Bitbucket: `GET /repositories/:workspace/`.
- [x] **Step 4:** Commit (`test: lock in GitLab groups + Bitbucket workspaces (D.2)`).

### Task 5: Confirm GitHub orgs pass-through

**Files:** `github.rs` + test.

- [x] Add a wiremock test that `list_organizations()` (GitHub `/user/orgs`) and `list_repositories_for_org` (`/orgs/:o/repos`) map correctly to `OrganizationDto`/`RemoteRepositoryDto`. If GitHub's `list_organizations` was relying on the default empty impl, implement it. Commit.
...
- [x] **Step 1:** Write a failing component test: given repos in groups "A"/"B", selecting group "A" in the filter shows only A's repos.
- [x] **Step 2:** Run → FAIL.
- [x] **Step 3:** Add a group filter dropdown (reuse the existing filter menu pattern already in `Repos/index.tsx`). Derive options from the distinct `repo.group` values. Filter the `repos` list by the selection (page-local state, persisted via `saveSettings` if a field is added — optional).
- [x] **Step 4:** Run → PASS. **Step 5:** Commit (`feat: group filter in repo list (D.2)`).
...
- [x] `cargo test --manifest-path app/src-tauri/Cargo.toml providers` green (GitLab/Bitbucket/GitHub author + org wiremock tests). _(39 passed, 0 failed)_
- [x] `yarn typecheck` green; `Repos.test.tsx` (group filter) green. Full `yarn lint && yarn test` not re-run as part of this audit — assumed covered by Phase-3 gate.
