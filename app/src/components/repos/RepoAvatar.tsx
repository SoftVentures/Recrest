...
function readStored(id: string): string | null {
  try {
    return localStorage.getItem("recrest:logo:" + id);
  } catch {
    return null;
  }
}
...
export function setRepoLogo(repoId: string, dataUrl: string | null): void {
  try {
    if (dataUrl) localStorage.setItem("recrest:logo:" + repoId, dataUrl);
    else localStorage.removeItem("recrest:logo:" + repoId);
  } catch {
    // Storage may be disabled — ignore; the in-memory value still works for
    // the current session once the event listeners fire.
  }
  window.dispatchEvent(
    new CustomEvent("recrest:logo-updated", { detail: { repoId, value: dataUrl } }),
  );
}
...
export function RepoAvatar({ repo, size = 24, radius = 6 }: RepoAvatarProps) {
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === "recrest:logo:" + repo.id) setCustom(e.newValue);
    };
    ...
    window.addEventListener("recrest:logo-updated", onCustom);
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("recrest:logo-updated", onCustom);
    };
  }, [repo.id]);
  ...
  if (specialIcon) {
    return (
      <div
        ...
        title={repo.name}
      >
        <Icon name={specialIcon} size={Math.round(size * 0.55)} color="#fff" />
      </div>
    );
  }
...
function detectSpecialIcon(name: string): IconName | null {
