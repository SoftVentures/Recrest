...
- **Never call `Command::new` for an external program directly.** Use `platform::host_command::{host_command, host_command_async}` and `host_which` instead. Recrest is packaged as a Flatpak, and inside that sandbox the user's `git`, IDE and terminal emulator live on the host where the sandbox cannot see them — a plain spawn fails, and a plain `which::which` reports "not installed" before any spawn happens. The wrapper routes through `flatpak-spawn --host` when sandboxed and is a **no-op everywhere else**, so there is no platform branch to write. The one sanctioned exception is `git_ops`'s `xdg-open`, which the OpenURI portal answers correctly on its own; it carries a comment saying so.
...
**`tauri:dev`** passes `--config src-tauri/tauri.dev.conf.json`, a minimal overlay that swaps `bundle.icon` to point at `icons-dev/`. Only `tauri dev` picks it up; `tauri build` ignores the overlay and keeps the production icon. Do not duplicate other fields in the overlay — keep it strictly about the icon swap.

**Never put `app.windows[]` in the overlay.** Tauri merges configs with JSON Merge Patch (RFC 7396, `tauri-utils/src/config/parse.rs`), which _replaces_ arrays instead of merging them by label — so any `windows[]` there silently discards the whole `tauri.macos.conf.json` entry. That is not theoretical: an overlay carrying `decorations: false` made the runtime `set_decorations(true)` in `lib.rs` actually fire, and `tao` implements it by rebuilding the style mask as `Titled|Closable|Miniaturizable|Resizable` — no `FullSizeContentView` — via `set_style_mask_async`, which lands _after_ `set_title_bar_style(Overlay)` and strips the bit that pulls the webview under the title bar. Result: two stacked title bars and square corners, in dev only. Window chrome on macOS is owned by `tauri.macos.conf.json` alone. The dev EXE-resource icon (Windows) is wired via `build.rs::WindowsAttributes::window_icon_path("icons-dev/windows/icon.ico")` so the taskbar shows the dev variant from process start.
